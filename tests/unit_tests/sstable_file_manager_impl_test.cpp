...
TEST_F(SSTableFileManagerTest, ConstructorCreatesFileInDirectory) {
    ...
    SSTableFileManagerImpl fm(tmp.path.string(), sysCtx); // we need to provide it with a mock SystemContext, with the mock constructor
    // assert: directory is no longer empty

    // after: exactly 1 file in that directory
    int count = 0;
    for (auto& p : fs::directory_iterator(tmp.path)) { count++; }
    EXPECT_EQ(count, 1);
