...
public:
    // ctors
    SSTableManagerImpl(std::string directoryPath, SystemContext &systemCtx, const std::vector<const Entry *> &entries); // use this to initialize, if the file doesn't exist yet. When this is called, it should create a new file in the directory.
    SSTableManagerImpl(const std::string &directoryPath, const std::string &fileName, SystemContext &systemCtx);        // initializes an SSTableFileManager with an existing fileName
